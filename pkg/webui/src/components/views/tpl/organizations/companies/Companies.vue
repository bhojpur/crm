    ...
    >
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title>
            <h1 style="font-size: 26px;">Категории товаров</h1>
          </v-toolbar-title>
      ...
      <template v-slot:item.product_count="{ item }">
        <span :class="{ 'red--text': !(item['products'] && item['products'].length > 0) }">
          {{item['products'] && item['products'].length > 0 ? item['products'].length : '0'}}
        </span>
      </template>
      ...
      <template v-slot:item.actions="{ item }">
        <div class="d-flex justify-space-between" style="width: 40px;">
          <v-icon
              @click="$router.push({name:'product-category.edit', params: {public_id:item['public_id']}})"
  ...
  export default {
    data: () => ({
      headers: [
        ...
        { text: 'Код',  value: 'code'},
        { text: 'Приоритет',  align: 'center', value: 'priority'},
        { text: 'Товаров', align: "center", value: 'product_count'},
    ...
    methods: {
      getDataFromApi: async function(showNotification = false, searchStr = '') {
        ...
        this.$api.company.getListPagination(payload)
                .then(resp => {
      ...
      createItem: async function (){
        ...
        await this.$api.company.create(payload)
                .then(async (data) => {
                  if (data['product_category']) {
                    if (!this.items) {
                      this.items = []
                    }
                    this.items.push(data['product_category'])
                  }
      ...
      deleteItemDialog() {
        ...
        this.$api.company.delete(payload)
                .then(async ()=> {
