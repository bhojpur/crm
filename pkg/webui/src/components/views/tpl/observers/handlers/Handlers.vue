    ...
    >
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title><h1 style="font-size: 26px;">Список обработчиков</h1></v-toolbar-title>
          <v-spacer></v-spacer>
      ...
      <template v-slot:item.actions="{ item }" >
        ...
        <v-btn @click="openDeleteDialog(item)" small depressed text fab>
        ...
        <td :colspan="headers.length">
          <v-row>
            <v-col cols="5">
              ...
              <v-text-field v-model.trim="item['name']" label="name" dense class="body-1" />
            </v-col>
            <v-col cols="6" offset="1">
              <v-textarea v-model.trim="item['description']" label="description" dense class="body-1" />
            </v-col>
          </v-row>

          <v-card flat style="background-color: transparent;" class="d-flex justify-end mb-3">
            <v-btn @click="openDeleteDialog(item)" color="red darken-3" elevation="1" small dark>
      ...
      <v-card>
        ...
        <v-card-text class="d-flex flex-column">
          <v-list>
            ...
            <v-list-item class="px-0 mx-0 mb-0 pb-0">
              <v-text-field label="Фамилия"  class="body-1" v-model.trim="dialogCreateItem['description']" aria-required="true" />
            </v-list-item>
  ...
  export default {
    ...
    methods: {
      getDataFromApi: async function(showNotification, searchStr = '') {
        ...
        await this.$api.eventHandlerItem.getListPagination(payload)
                .then(resp => {
                  if (resp['handler_items'] !== undefined ) {
                    this.items = resp['handler_items']
      ...
      updateItemData: async function (item) {
        ...
        await this.$api.eventHandlerItem.update(payload)
                .then((resp) => {
                  if (resp['handler_item'] !== undefined) {
                    item = this.items.find(el => el.id === resp['handler_item'].id)
                    if (item !== undefined && item !== null) item = resp['handler_item']
                    this.$notify({
                      group: 'main',
                      title: 'Данные обновлены',
                      type: 'success'
                    ...
                    this.$notify({
                      ...
                      text: 'не хватает объекта [handler_item]',
                      type: 'warring'
                    });
                  }
                })
      ...
      createItem: async function (){
        ...
        await this.$api.eventHandlerItem.create(payload)
                .then((resp) => {
                  if (resp['handler_item'] !== undefined) {
                    this.items.push(resp['handler_item'])
                    this.$notify({
                      group: 'main',
                      title: 'Объект создан',
                      type: 'success'
      ...
      deleteItemDialog: async function() {
        ...
        await this.$api.eventHandlerItem.delete(payload)
                .then(() => {
    ...
    computed: {
      headers() {
        let _h = [
          ...
          { text: 'Глобальный статус',  value: 'enabled'},
          { text: 'Добавлен', value: 'created_at', sortable: false},
        ]
        if (this.isMainAccount) {
          _h.unshift({ text: 'Id', align: 'start', value: 'id'})
          _h.push({ text: 'Удалить', align: 'center', value: 'actions', sortable: false})
        }
        return _h
      }
    }
