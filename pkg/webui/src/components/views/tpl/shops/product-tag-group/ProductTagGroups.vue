    ...
    >
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title>
            <h1 style="font-size: 26px;">Группы тегов товаров</h1>
          </v-toolbar-title>
      ...
      <template v-slot:item._tag_count="{ item }">
        <span
        :class="{ 'deep-orange--text': item._tag_count < 1 }" class="mr-3"
        v-text="item._tag_count"></span>
      </template>

      <template v-slot:item.actions="{ item }">
        <show-action :route="{name:'product-tag-group.edit', params: {public_id:item['public_id']}}" @delete="openDeleteDialog(item)"></show-action>
      </template>
  ...
  export default {
    data: () => ({
      headers: [
        ...
        { text: 'Код',  value: 'code'},
        { text: 'Filter label',  value: 'filter_label'},
        { text: 'Filter code',  value: 'filter_code'},
        { text: 'Число тегов',  align: 'end',value: '_tag_count', sortable: false},
    ...
    methods: {
      getDataFromApi: async function(showNotification = false) {
        ...
        this.$api.productTagGroup.getListPagination(payload)
                .then(resp => {
                  if (resp['product_tag_groups'] !== undefined ) {
                    ...
                    if (showNotification) {
                      this.$notify({
                        ...
                        text: 'Ошибка в ответе сервера: product_tag_groups - not found',
                        type: 'warring'
                      });
                    }
                  }
      ...
      createItem: async function (){
        ...
        await this.$api.productTagGroup.create(payload)
                .then(async (data) => {
                  if (data['product_tag_group']) {
                    if (!this.items) {
                      this.items = []
                    }
                    this.items.push(data['product_tag_group'])
                  }
                  await this.getDataFromApi(false)
                  this.$notify({
                    group: 'main',
                    title: 'Категория товара создана',
                    type: 'success'
                  });
                })
                .catch( (err) => {
                  this.$notify({
                    ...
                    text: 'Не удалось создать категорию товара',
                    type: 'error'
                  });
                })
      },
      ...
      deleteItemDialog() {
        ...
        this.$api.productTagGroup.delete(payload)
                .then(async ()=> {
