  ...
  <v-container fluid>
    <v-card class="pa-2" style="border-top-left-radius: 2px;border-top-right-radius: 2px;">
      ...
      <v-toolbar color="white" flat>
        <v-toolbar-title class="d-flex flex-column pt-6">
          <h1 style="font-size: 26px;">Поставка товаров</h1>
          <v-btn @click="$router.push({name: 'shipments.list'})" class="ml-0 pl-0" outlined small text>
    ...
    <v-data-table
        ...
        :headers="[
          ...
          { text: 'Розничная цена', value: 'product.retail_price',align: 'end', sortable: false},
          { text: 'Отпуск',  align: 'center',value: 'enabled'},
          { text: 'Действия', value: 'actions', sortable: false},
        ]"
        :items.sync="item.shipment_items"
        class="elevation-1"
        item-key="id"
    >
      <template v-slot:top>
...
export default {
  ...
  methods: {
    uploadItem: async function (showNotification = false) {
      this.loading = true
      await this.$api.shipment.getByPublicId({
        accountHashId: this.$store.state.account.hash_id,
        public_id: this.$route.params.public_id,
        preloads:"ShipmentItems.Product", // может надо отдельно подгружать карточки товаров через filter[]
      })
          .then(resp => {
            if (resp['shipment'] !== undefined) {
              this.item = resp['shipment']
              if (!resp['shipment']['shipment_items']) this.item['shipment_items'] = []
              else this.item['shipment_items'] = resp['shipment']['shipment_items']
              if (showNotification) {
                this.$notify({
                  group: 'main',
    ...
    updateItemData: async function () {
      ...
      Object.assign(payload, {
        accountHashId: this.$store.state.account.hash_id,
        preloads:"ShipmentItems.Product",
      })

      await this.$api.shipment.update(payload)
          .then((resp) => {
            if (resp['shipment'] !== undefined) {
              this.item = resp['shipment']
              ...
              this.$notify({
                ...
                text: 'не хватает объекта [shipment]',
                type: 'warring',
              });
            }
          })
    ...
    choiceProduct: async function (product) {
      ...
      this.$api.shipment.appendProduct(payload)
          .then(async (resp) => {
            if (resp['shipment']) {
              ...
              this.$notify({
                group: 'main',
                title: 'Ошибка обновления',
                text: 'Отсутствует переменная в ответе shipment',
                type: 'warring',
              });
            }
    ...
    removeProduct: async function (warehouseItem) {
      ...
      this.$api.shipment.removeProduct(payload)
          .then(async (resp) => {
            if (resp['shipment']) {
    ...
    setNewStatus: async function (status) {
      ...
      this.loading = true
      await this.$api.shipment.changeStatus({
        accountHashId: this.$store.state.account.hash_id,
        id: this.item.id,
        status: status,
        preloads:"ShipmentItems.Product",
